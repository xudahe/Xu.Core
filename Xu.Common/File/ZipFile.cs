...
namespace Xu.Common
{
    ...
    //{
    //    //NUGET 安装 SharpZipLib

    //    ZipFile zc = new ZipFile();
    //    string webRootPath = _webHostEnvironment.ContentRootPath + "\\A\\";//压缩文件目录

    //    //压缩后的Zip文件路径
    //    var zippath = webRootPath + "1.zip";

    //    if (System.IO.File.Exists(zippath))//如果压缩包存在删除已有压缩文件
    //    {
    //        System.IO.File.Delete(zippath);
    //    }

    //    //需要压缩的文件夹
    //    zc.CreateZip(webRootPath, zippath);

    //    return File(zc.FileContent(zippath), "application/octet-stream", "附件.zip");
    //}
