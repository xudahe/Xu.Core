    ...
    public class SchedulerCenterServer : ISchedulerCenter
    {
        ...
        public async Task<MessageModel<string>> AddScheduleJobAsync(TasksQz tasksQz)
        {
            ...
            if (tasksQz != null)
            {
                try
                {
                    ...
                    //    .WithIdentity(tasksQz.Name, tasksQz.JobGroup)
                    //    .Build();
                    ...
                    result.Success = true;
                    result.Message = $"启动任务:【{tasksQz.Name}】成功";
                    return result;
                }
                catch (Exception ex)
                {
        ...
        public async Task<MessageModel<string>> StopScheduleJobAsync(TasksQz tasksQz)
        {
            ...
            try
            {
                ...
                {
                    result.Success = false;
                    result.Message = $"未找到要暂停的任务:【{tasksQz.Name}】";
                    return result;
                }
                else
                {
                    ...
                    result.Success = true;
                    result.Message = $"暂停任务:【{tasksQz.Name}】成功";
                    return result;
                }
            }
            catch (Exception)
        ...
        public async Task<MessageModel<string>> ResumeJob(TasksQz tasksQz)
        {
            ...
            try
            {
                ...
                {
                    result.Success = false;
                    result.Message = $"未找到要重新的任务:【{tasksQz.Name}】,请先选择添加计划！";
                    return result;
                }
                await this._scheduler.Result.ResumeJob(jobKey);
                result.Success = true;
                result.Message = $"恢复计划任务:【{tasksQz.Name}】成功";
                return result;
            }
            catch (Exception)
            {
